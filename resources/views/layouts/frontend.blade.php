            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
                ...
                <div class="text-center md:text-left">
                    <div class="bg-white p-4 rounded-lg inline-block mb-4">
                        <span class="text-blue-700 font-bold text-xl flex flex-col items-center">
                            @if ($branding?->logo_path)
                                <img src="{{ asset('storage/' . $branding->logo_path) . ($v ? '?v=' . $v : '') }}"
                                    alt="Logo" class="mb-1 object-contain"
                                    style="{{ $branding?->logo_width ? 'width:' . $branding->logo_width . 'px;' : 'width: 32px;' }}{{ $branding?->logo_height ? 'height:' . $branding->logo_height . 'px;' : '' }}" />
                            @else
                    ...
                    <div class="flex justify-center md:justify-start space-x-4">
                        @if ($contactSetting?->instagram)
                            <!-- Instagram -->
                            <a href="{{ $contactSetting->instagram }}"
                        ...
                        @if ($contactSetting?->facebook)
                            <!-- Facebook -->
                            <a href="{{ $contactSetting->facebook }}"
                                class="text-white hover:text-blue-200 transition-colors">
                        ...
                        @if ($contactSetting?->twitter)
                            <!-- Twitter / X -->
                            <a href="{{ $contactSetting->twitter }}"
                                class="text-white hover:text-blue-200 transition-colors">
                                <span class="sr-only">Twitter</span>
                                <svg class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                                        d="M13.6823 10.6218L20.2391 3H18.6854L12.9921 9.61788L8.44486 3H3.2002L10.0765 13.0074L3.2002 21H4.75404L10.7663 14.0113L15.5685 21H20.8131L13.6819 10.6218ZM11.5541 13.0956L10.8574 12.0991L5.31391 4.16971H7.70053L12.1742 10.5689L12.8709 11.5655L18.6861 19.8835H16.2995L11.5541 13.096V13.0956Z" />
                                </svg>
                            </a>
                        @endif

                        @if ($contactSetting?->youtube)
                            <!-- YouTube -->
                            <a href="{{ $contactSetting->youtube }}"
                                class="text-white hover:text-blue-200 transition-colors">
                        ...
                        @if ($contactSetting?->tiktok)
                            <!-- TikTok -->
                            <a href="{{ $contactSetting->tiktok }}"
                                class="text-white hover:text-blue-200 transition-colors">
                ...
                <div>
                    ...
                    @if ($footerSetting?->description)
                        <p class="text-blue-100 mb-4 leading-relaxed text-sm">
                            {{ $footerSetting->description }}
                        </p>
                    @endif
                    <ul class="space-y-4">
                        @foreach ($footerLinks->where('category', 'about_us') as $link)
                            <li><a href="{{ $link->url }}"
                                    class="text-blue-100 hover:text-white text-base">{{ $link->title }}</a></li>
                        @endforeach
                    </ul>
                </div>
                ...
                <div>
                    ...
                    <ul class="space-y-4">
                        @foreach ($footerLinks->where('category', 'related') as $link)
                            <li><a href="{{ $link->url }}"
                                    class="text-blue-100 hover:text-white text-base">{{ $link->title }}</a></li>
                        @endforeach
                    </ul>
                ...
                <div>
                    ...
                    <p class="text-blue-100 mb-4 leading-relaxed">
                        {{ $contactSetting?->address ?? 'Alamat belum diatur' }}
                    </p>
                    <p class="text-blue-100">{{ $contactSetting?->phone ?? 'Telepon belum diatur' }}</p>
                    @if ($contactSetting?->email)
                        <p class="text-blue-100 mt-2">{{ $contactSetting->email }}</p>
                    @endif
                </div>
            </div>
